<template>
  <v-detail-dialog show-full-screen :fixed-body-height="false" use-body-scrolling title="查看报警信息" v-model="visible"
    :loading="loading" :disableFooter="true" @cancel="cancelConfirm" @opened="handleOpened">
    <el-form :model="formData" label-width="100px" ref="formRef">
      <el-row>
        <el-col :span="12">
          <el-form-item label="事件名称：" prop="eventName">
            <el-input v-model="formData.eventName" autocomplete="off" readonly placeholder="请输入事件名称"
              style="width: 300px" />
          </el-form-item>
        </el-col>
        <el-col :span="12">
          <el-form-item label="处理人员：" prop="handleIds">
            <el-select placeholder="请选择处理人员" multiple disabled v-model="formData.handleIds" style="width: 300px">
      ...
      <el-form-item label="事件内容：" prop="content">
        <el-input v-model="formData.content" autocomplete="off" readonly placeholder="请输入事件内容" :rows="2" type="textarea"
          style="width: 760px" />
      </el-form-item>
      <el-form-item label="事件类型：" prop="type">
        <el-input v-model="formData.type" readonly placeholder="请输入事件类型" />
      </el-form-item>
      <el-form-item label="处理流程：" prop="processingFlow">
        <el-input v-model="formData.processingFlow" autocomplete="off" readonly placeholder="请输入处理流程" :rows="2"
          type="textarea" style="width: 760px" />
      </el-form-item>
      <!-- v-if="formData.emergencyAlarmDTOs != null && formData.emergencyAlarmDTOs.length > 0" -->
      <el-form-item label="报警描述：" v-if="formData.emergencyAlarmDTOs!=null">
        <el-text type="danger" v-for="text in formData.emergencyAlarmDTOs">{{ text.description }}</el-text>
      </el-form-item>
      <el-form-item v-if="paths != null && paths.length >0" label="参考文件：" prop="emergencyAlarmIds">
        <!-- <el-select
          placeholder="请选择报警信息"
          multiple
          disabled
